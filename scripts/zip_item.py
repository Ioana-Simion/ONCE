...
if UserK is None or select_num is None or alpha is None:
    assert (
        UserK is None and select_num is None and alpha is None
    ), "UserK, select_num, alpha must be set together"
    user_key = ""
    key = f"ItemK{ItemK}"
...
item_embed_path = (
    "../saving/MIND-small-v2/NAML/free-train_get_item_embedding/item_embeddings.npy"
)
...
item_center_fut = (
    ...
    .construct()
    .store(os.path.join(item_depot_export_base_dir, f"item_{key}"))
)
...
train_depot.filter(bool, col="click").filter(
    lambda x: x in center_items, col="nid"
).reset_index().export(os.path.join(item_depot_export_base_dir, f"train_{key}"))

dev_depot.filter(lambda x: x in center_items, col="nid").reset_index().export(
    os.path.join(item_depot_export_base_dir, f"dev_{key}")
)
...
for sample in target_depot:
    ...
    while len(user_neg) < 20 + random.randint(1, 5):
        ...
        if (
            news_depot[nid]["cat"] not in taste
            and node_to_center[nid] not in center_taste
        ):
            user_neg.add(nid)
