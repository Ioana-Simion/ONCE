...
class MajorityUser:
    def __init__(self, meta: DataMeta):
        ...
        self.store_dir = os.path.join(self.meta.base_path, "majority_user")
    ...
    def majority_choice(self, k):
        ...
        ).reset_index().export(os.path.join(self.store_dir, f"train_{key}_v2"))

        self.dev_depot.filter(
            lambda x: x in selected_users, col="uid"
        ).reset_index().export(os.path.join(self.store_dir, f"dev_{key}_v2"))
