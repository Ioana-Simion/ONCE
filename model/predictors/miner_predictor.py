...
class MINERPredictorConfig(BasePredictorConfig):
    def __init__(self, score_type: str = "weighted", **kwargs):
...
class TargetAwareAttention(nn.Module):
    ...
    def forward(self, query: torch.Tensor, key: torch.Tensor, value: torch.Tensor):
        ...
        weights = F.softmax(
            torch.matmul(key, proj.permute(0, 2, 1)), dim=2
        )  # batch_size, K+1, num_context_codes
...
class MINERPredictor(BasePredictor):
    ...
    def predict(self, user_embeddings, item_embeddings):
        ...
        scores = torch.matmul(
            item_embeddings, user_embeddings.permute(0, 2, 1)
        )  # batch_size, K+1, num_context_codes
        if self.config.score_type == "weighted":
            return self.target_aware_attention.forward(
                query=user_embeddings, key=item_embeddings, value=scores
            )
