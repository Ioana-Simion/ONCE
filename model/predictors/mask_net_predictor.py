...
class MaskNetPredictor(BasePredictor):
    ...
    def __init__(self, **kwargs):
        ...
        if self.config.sequential_mode:
            ...
            self.mask_net = ParallelMaskNet(
                config=self.config, input_dim=self.config.hidden_size * 2
            )
        if self.config.embed_layer_norm:
            self.emb_norm = nn.ModuleList(
                nn.LayerNorm(self.config.hidden_size) for _ in range(2)
            )
        else:
            self.emb_norm = None
    ...
    def predict(self, user_embeddings, item_embeddings):
        ...
        return self.mask_net.forward(
            input_embeddings.flatten(start_dim=1), hidden_states.flatten(start_dim=1)
        ).flatten()
