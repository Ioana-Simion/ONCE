        ...
        **kwargs,
        ...
        if self.use_item_content:
            if not self.item_config:
                ...
                pnt(
                    "automatically set item_config to an empty dict, as use_item_content is True"
                )
...
class Legommender(BaseModule):
    def __init__(
        ...
        self.loss_func = (
            nn.CrossEntropyLoss() if self.use_neg_sampling else nn.BCEWithLogitsLoss()
        )
    ...
    def get_item_content(self, batch, col):
        ...
        if not self.llm_skip:
            _shape = None
            item_content = self.shaper.transform(
                batch[col]
            )  # batch_size, click_size, max_seq_len
        ...
        item_contents = self.item_encoder.get_full_placeholder(sample_size).to(
            Meta.device
        )
    ...
    def forward(self, batch):
        if (
            isinstance(batch[self.candidate_col], torch.Tensor)
            and batch[self.candidate_col].dim() == 1
        ):
        ...
        if self.config.use_item_content:
            ...
            item_embeddings = self.embedding_manager(self.clicks_col)(
                batch[self.candidate_col].to(Meta.device)
            )
        ...
        if self.status.is_testing or (
            self.status.is_evaluating and not Meta.simple_dev
        ):
            return scores
    ...
    def predict_for_neg_sampling(self, item_embeddings, user_embeddings):
        ...
        user_embeddings = self.user_encoder.prepare_for_predictor(
            user_embeddings, candidate_size
        )
    ...
    def prepare_item_module(self):
        item_config = self.item_encoder_class.config_class(
            **combine_config(
