...
class SemanticInputer(BaseInputer):
    ...
    def __init__(self, item_hub: DataHub, **kwargs):
        ...
        assert (
            len(self.order) == 1
        ), "semantic inputer only support one column of user history"
    ...
    def get_empty_input(self):
        return torch.zeros(
            self.max_sequence_len, self.max_content_len, dtype=torch.long
        )
    ...
    def sample_rebuilder(self, sample: OrderedDict):
        ...
        attention_mask = torch.tensor(
            [1] * len(items) + [0] * (self.max_sequence_len - len(items)),
            dtype=torch.long,
        )
