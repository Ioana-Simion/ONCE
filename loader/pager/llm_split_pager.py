...
class LLMSplitPager(BasePager):
    def __init__(
        self, inputer: NaturalConcatInputer, layers: list, hidden_size: int, **kwargs
    ):
        ...
        self.final_masks = torch.zeros(len(self.contents), inputer.max_sequence_len)
    ...
    def store(self, store_dir):
        ...
        for index, layer in enumerate(self.layers):
            np.save(
                os.path.join(store_dir, f"layer_{layer}.npy"),
                self.final_features[index].numpy(),
            )
        np.save(os.path.join(store_dir, "mask.npy"), self.final_masks.numpy())
