...
class EmbeddingHub:
    ...
    def load_pretrained_embedding(self, vocab_name, **kwargs):
        ...
        pnt(
            f"load pretrained embedding {vocab_name} of {self._pretrained[vocab_name].embedding.shape}"
        )
    ...
    def build_vocab_embedding(self, vocab_name, vocab_size):
        ...
        if vocab_name in self._pretrained:
            ...
            if int(embedding_weights.shape[0]) != vocab_size:
                raise ValueError(
                    f"{vocab_name} not meet the expected vocab size {vocab_size}"
                )
            ...
            if embedding_weights.dim() == 3:
                ...
                if embedding_size != self.hidden_size or self.same_dim_transform:
                    pnt(
                        f"transform hidden size from {embedding_size} to {self.hidden_size}"
                    )
                    embedding = TransformEmbedding(
                        embedding=embedding, to_dim=self.hidden_size
                    )
                else:
        ...
        self._table.add_module(
            vocab_name,
            nn.Embedding(num_embeddings=vocab_size, embedding_dim=self.hidden_size),
        )
