...
class Depots:
    def __init__(self, user_data, modes: set, column_map: ColumnMap):
        ...
        if Phases.train in modes:
            self.train_depot = DepotHub.get(
                user_data.depots.train.path, filter_cache=user_data.filter_cache
            )
        if Phases.dev in modes:
            self.dev_depot = DepotHub.get(
                user_data.depots.dev.path, filter_cache=user_data.filter_cache
            )
        if Phases.test in modes:
            self.test_depot = DepotHub.get(
        ...
        if user_data.allowed:
            ...
            for phase in self.depots:
                ...
                super(CachingDep, depot).filter(
                    lambda x: x in allowed_list, col=depot.id_col
                )
                pnt(
                    f"Filter {phase} phase with allowed list, sample num: {sample_num} -> {len(depot)}"
                )
        ...
        for phase in [Phases.train, Phases.test]:
            ...
            for col in filters:
                for filter_str in filters[col]:
                    ...
                    depot.filter(filter_func_str, col=col)
                    pnt(
                        f"Filter {col} with {filter_str} in {phase} phase, sample num: {len(depot)}"
                    )
    ...
    def negative_filter(self, col):
        ...
        for phase in phases:
            ...
            pnt(
                f"Filter {col} with x==1 in {phase} phase, sample num: {sample_num} -> {len(depot)}"
            )
