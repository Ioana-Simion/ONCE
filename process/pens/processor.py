...
from UniTok.tok import BertTok, EntTok, IdTok, NumberTok, SplitTok
...
class Processor:
    ...
    def tokenize(self):
        ...
        news_tok.read(self.news_df).tokenize().store(
        ...
        user_tok.read(self.user_df).tokenize().store(
    ...
    def tokenize_test(self):
        ...
        user_tok.read(user_df).tokenize().store(
            os.path.join(self.store_dir, "user-test")
        )
        ...
        inter_tok = self.get_inter_tok()
        inter_tok.read(fake_inter_data).tokenize().store(
