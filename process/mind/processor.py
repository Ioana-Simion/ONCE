...
from UniTok.tok import BaseTok, BertTok, EntTok, IdTok, SplitTok
...
class Processor:
    def __init__(
        self, data_dir, store_dir, glove=None, imp_list_path: str = None, v2=True
    ):
    ...
    def get_news_tok(self, max_title_len=0, max_abs_len=0):
        ...
        return (
            ...
            .add_col(Column(name="cat", tokenizer=EntTok(name="cat").as_sing()))
            .add_col(
                Column(
                    name="subcat",
    ...
    def get_user_tok(self, max_history: int = 0):
        ...
        user_ut.add_col(
            ...
            Column(
                name="history",
                tokenizer=SplitTok(name="history", sep=" ", vocab=self.nid).as_list(
                    max_length=max_history, slice_post=True
                ),
            )
        )
        return user_ut
    ...
    def get_neg_tok(self, max_neg: int = 0):
        ...
        neg_ut.add_col(
            ...
            Column(
                name="neg",
                tokenizer=SplitTok(name="neg", sep=" ", vocab=self.nid).as_list(
                    max_length=max_neg, slice_post=True
                ),
            )
        )
        return neg_ut
    ...
    def reassign_inter_df(self):
        ...
        return (
            pd.concat(inter_train_df, ignore_index=True),
    ...
    def tokenize(self):
        ...
        user_tok.read_file(user_df).tokenize().store_data(
    ...
    def tokenize_neg(self):
        ...
        neg_tok.read_file(neg_df).tokenize().store_data(
