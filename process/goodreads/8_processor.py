...
from UniTok.tok import BertTok, EntTok, IdTok, NumberTok, SeqTok
...
class Processor:
    ...
    def get_user_tok(self):
        return (
            ...
            .add_col(Column(name="history", tok=SeqTok(vocab=self.book_vocab)))
        )
    ...
    def get_neg_tok(self):
        return (
            ...
            .add_col(Column(name="neg", tok=SeqTok(vocab=self.book_vocab)))
        )
    ...
    def tokenize(self):
        ...
        book_tok.read(book_data).tokenize().store_data(
            os.path.join(self.store_dir, "book")
        )

        user_data = self.read_user_data()
        user_tok = self.get_user_tok()
        user_tok.read(user_data).tokenize().store_data(
            os.path.join(self.store_dir, "user")
        )

        neg_data = self.read_neg_data()
        neg_tok = self.get_neg_tok()
        neg_tok.read(neg_data).tokenize().store_data(
            os.path.join(self.store_dir, "neg")
        )

        train_data = self.read_inter_data("train")
        train_tok = self.get_inter_tok()
        train_tok.read(train_data).tokenize().store_data(
            os.path.join(self.store_dir, "train")
        )

        valid_data = self.read_inter_data("dev")
        valid_tok = self.get_inter_tok()
        valid_tok.read(valid_data).tokenize().store_data(
            os.path.join(self.store_dir, "dev")
        )

        test_data = self.read_inter_data("test")
        test_tok = self.get_inter_tok()
        test_tok.read(test_data).tokenize().store_data(
            os.path.join(self.store_dir, "test")
        )
