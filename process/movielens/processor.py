...
class Processor:
    ...
    def tokenize(self):
        self.get_inter_tok().read(self.train_df).tokenize().store(
            os.path.join(self.store_dir, "train")
        )
        self.get_inter_tok().read(self.dev_df).tokenize().store(
            os.path.join(self.store_dir, "dev")
        )
        self.get_inter_tok().read(self.test_df).tokenize().store(
            os.path.join(self.store_dir, "test")
        )

        self.get_history_tok().read(self.history_df).tokenize().store(
            os.path.join(self.store_dir, "user")
        )

    def tokenize_item(self):
        self.get_item_tok().read(self.item_df).tokenize().store(
            os.path.join(self.store_dir, "item")
        )
    ...
    def neg_sample(self):
        ...
        self.get_neg_tok().read(df).tokenize().store(
