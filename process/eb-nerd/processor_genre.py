...
class Processor:
    ...
    def read_inters(self, mode='train'):
        ...
        if mode == 'train':
            ...
            df = pd.read_parquet(
                path=os.path.join(self.data_dir, mode, 'behaviors_with_user_interests.parquet'),
                columns=columns
            )
        else:
            df = pd.read_parquet(
    ...
    def get_inter_tok(self, mode='train'):
        tok = UniTok().add_index_col(
            name='index'
        ).add_col(Column(
            name='imp',
            tok=EntTok,
    ...
    def tokenize(self, load_news=False, load_user=False):
        ...
        inter_tok = self.get_inter_tok('train')
        ...
        inter_tok = self.get_inter_tok('validation')
